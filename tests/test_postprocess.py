...
class TestQuantize(unittest.TestCase):
    ...
    def count_colors(self, path: Path) -> int:
        with Image.open(path) as im:
            ...
            if len(palette) % 4 != 0:
                self.fail(f"Palette is {len(palette)} long which is not a multiple of 4")
            return int(len(palette) / 4)
    ...
    def test_quantize(self) -> None:
        with tempfile.TemporaryDirectory() as workdir_str:
            ...
            self.assertEqual(self.count_colors(q64), 85)
            ...
            self.assertEqual(self.count_colors(q64d), 85)
